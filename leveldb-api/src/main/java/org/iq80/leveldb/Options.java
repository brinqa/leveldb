...
public class Options {
     ...
     * database has a large working set (budget one open file per 2MB of working set).
     */
    ...
    /**
     * Leveldb will write up to this amount of bytes to a file before switching to a new one. Most
     * clients should leave this parameter alone. However if your filesystem is more efficient with
     * larger files, you could consider increasing the value. The downside will be longer
     ...
     * dynamically.
     *
     * <p>Default: {@link CompressionType#LZ4}, which gives lightweight but fast compression.
     *
     * <p>Typical speeds of {@link CompressionType#LZ4} on an Intel(R) Core(TM)2 2.4GHz:
     * ~200-500MB/s compression ~400-800MB/s decompression Note that these speeds are significantly
     * faster than most persistent storage speeds, and therefore it is typically never worth
     * switching to {@link CompressionType#NONE}. Even if the input data is incompressible, the
     * {@link CompressionType#LZ4} implementation will efficiently detect that and will switch to
     * uncompressed mode.
     *
     * <p>If Snappy compression is not available, {@link CompressionType#NONE} will be used.
     */
    ...
    /**
     * If {@code cacheSize} is zero, no block cache will be used- If non-zero, use the specified
     * cache size for blocks. By default leveldb will automatically create and use an 8MB internal
     * cache.
     */
     ...
     *
     * <p>Comparator used to define the order of keys in the table. Default: a comparator that uses
     * lexicographic byte-wise ordering
     *
     * <p>REQUIRES: The client must ensure that the comparator supplied here has the same name and
    ...
    /**
     * Any internal progress/error information generated by the db will be written to {@code logger}
     * if it is non-null, or to a file stored in the same directory as the DB contents if info_log
     * is null.
     */
     ...
     * corruption of one DB entry may cause a large number of entries to become unreadable or for
     ...
     * benefit from passing an instance of BloomFilter
     *
    ...
    /**
     * If true, append to existing MANIFEST and log files when a database is opened. This can
     * significantly speed up open.
     */
