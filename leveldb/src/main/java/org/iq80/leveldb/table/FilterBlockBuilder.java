...
/**
 * The filter block stores a sequence of filters, where filter i contains the output of
 * FilterPolicy::CreateFilter() on all keys that are stored in a block whose file offset falls
 * within the range
 *
 * <p>[ i*base ... (i+1)*base-1 ]
 *
 * <p>Currently, "base" is 2KB. So for example, if blocks X and Y start in the range [ 0KB .. 2KB-1
 * ], all of the keys in X and Y will be converted to a filter by calling
 * FilterPolicy::CreateFilter(), and the resulting filter will be stored as the first filter in the
 * filter block.
 *
 * <p>The filter block is formatted as follows:
 *
 * <p>[filter 0] [filter 1] [filter 2] ... [filter N-1]
 *
 * <p>[offset of filter 0] : 4 bytes [offset of filter 1] : 4 bytes [offset of filter 2] : 4 bytes
 * ... [offset of filter N-1] : 4 bytes
 *
 * <p>[offset of beginning of offset array] : 4 bytes lg(base) : 1 byte
 *
 * <p>
 *
 * <p>
