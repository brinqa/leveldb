...
/**
 * If for some reason {@link #build()} is not called (eg: due to some exception) when {@link
 * SafeListBuilder#close()} is called, then {@link Closeable#close()} method will be called on each
 * inserted items.
 *
...
public final class SafeListBuilder<T extends Closeable> implements Closeable {
    ...
    /**
     * Returns a newly-created {@code ImmutableList} based on the contents of the {@code
     * SafeListBuilder}.
     *
     * <p>After this call, {@link #close()} wont have any effect on item of the list.
     */
    public List<T> build() {
    ...
    /**
     * If {@link #build()} was not called yet, {@link Closeable#close()} method will be called on
     * all items added trough {@link #add(Closeable)} and {@link #addAll(Iterable)}. If {@link
     * #build()} was already used, nothing will happen on the data of this builder.
     */
    @Override
