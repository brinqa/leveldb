...
public class GIssue320Test {
    ...
    public void testRaiseFromTheDead() throws IOException {
        ...
        Consumer<WriteBatchImpl> writeAndClear =
                b -> {
                    db.write(b);
                    b.clear();
                };
        ...
        for (int i = 0; i < 2000000; i++) {
            if (rand.nextInt(50000) == 0) {
                ...
                try (final Snapshot snapshot = db.getSnapshot();
                        final DBIterator it = db.iterator(new ReadOptions().snapshot(snapshot))) {
                    it.seek(new byte[] {'A'});
                    for (byte[] iterableDatum : dataToDelete) {
                        ...
                        if (!alwaysPresentData.isEmpty() && rand.nextInt(10) == 0) {
                            final byte[] key =
                                    alwaysPresentData.get(rand.nextInt(alwaysPresentData.size()));
                            assertNotNull(db.get(key));
                        }
                    }
                    i += dataToDelete.size();
                    if (it.hasNext()) {
                        // all "A" prefixed keys where deleted so only non A prefixed keys should
                        // exist
        ...
        iterableData.sort(
                (o1, o2) ->
                        bytewiseComparator.compare(
                                Slices.wrappedBuffer(o1), Slices.wrappedBuffer(o2)));
        byte[] p = null;
