...
public class InternalUserComparatorTest {
    ...
    public void testInternalKeyShortSeparator() {
        assertEquals(
                ...
                shorten(key("foo", 100, ValueType.VALUE), key("foo", 99, ValueType.VALUE)));
        assertEquals(
                key("foo", 100, ValueType.VALUE),
                shorten(key("foo", 100, ValueType.VALUE), key("foo", 101, ValueType.VALUE)));
        assertEquals(
                key("foo", 100, ValueType.VALUE),
                shorten(key("foo", 100, ValueType.VALUE), key("foo", 100, ValueType.VALUE)));
        assertEquals(
                key("foo", 100, ValueType.VALUE),
                shorten(key("foo", 100, ValueType.VALUE), key("foo", 100, ValueType.DELETION)));

        // When user keys are misordered
        assertEquals(
                key("foo", 100, ValueType.VALUE),
                shorten(key("foo", 100, ValueType.VALUE), key("bar", 99, ValueType.VALUE)));

        // When user keys are different, but correctly ordered
        assertEquals(
                key("g", SequenceNumber.MAX_SEQUENCE_NUMBER, valueTypeForSeek),
                shorten(key("foo", 100, ValueType.VALUE), key("hello", 200, ValueType.VALUE)));

        // When start user key is prefix of limit user key
        assertEquals(
                key("foo", 100, ValueType.VALUE),
                shorten(key("foo", 100, ValueType.VALUE), key("foobar", 200, ValueType.VALUE)));

        // When limit user key is prefix of start user key
        assertEquals(
                key("foobar", 100, ValueType.VALUE),
                shorten(key("foobar", 100, ValueType.VALUE), key("foo", 200, ValueType.VALUE)));
    }
    ...
    public void testInternalKeyShortestSuccessor() {
        ...
        assertEquals(
                key(new byte[] {(byte) 0xff, (byte) 0xff}, 100, ValueType.VALUE),
