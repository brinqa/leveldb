...
public class OutOfSpaceTest {
    ...
    /**
     * Write concurrently to db and fail at some point like an out of memory. Verify that all theads
     * fail not for invalid state but due to Out of memory exception. It is expected that DB should
     * not be usable after a write failure (in write request or compaction).
     */
    ...
    public void testAllWritesShouldFailAfterFirstWriteFailure(int diskSpace) throws Exception {
        ...
        // simulate concurrent work with random batch side, this will stimulate multi write grouping
        // into one batch
        // inside the DB
        final int threads = 4;
        ...
        try {
            ...
            for (Future<Exception> exceptionFuture : fut) {
                ...
                assertTrue(
                        rootCause.getMessage().equals("Out of space"),
                        "Out of space exception is expected as route cause of failure in "
                                + exception);
            }

            // DB should be failed with background failure, so any new write should fail with
            // background exception cause
            // last check to verify that if we try to write additional records to DB we get same
            // route cause twice
            ...
            Throwable t1 =
                    Throwables.getRootCause(Assert.expectThrows(Exception.class, shouldFail));
            Throwable t2 =
                    Throwables.getRootCause(Assert.expectThrows(Exception.class, shouldFail));
            assertSame(t1, t2);
            assertNotNull(t1, "Route cause is expected in thrown exception" + t1);
            assertTrue(
                    t1.getMessage().equals(OUT_OF_SPACE),
                    "Out of space exception is expected as route cause of failure in " + t1);
        } finally {
