...
public class MemTableIteratorTest {
    List<Map.Entry<InternalKey, Slice>> sampleEntries1 =
            Lists.newArrayList(
                    entry("k1", 102, ValueType.VALUE, "v2"),
                    entry("k1", 100, ValueType.VALUE, "v1"),
                    entry("k2", 100, ValueType.VALUE, "v1"),
                    entry("k3", 300, ValueType.VALUE, "v1"),
                    entry("k4", 100, ValueType.VALUE, "v1"));
    ...
    public void testForwardScanAfterSeek() throws Exception {
        ...
        for (boolean valid = iter.seek(new InternalKey(sampleEntries1.get(2).getKey().encode()));
                valid;
                valid = iter.next()) {
    ...
    private void validateAll(
            InternalIterator iter, boolean valid1, List<Map.Entry<InternalKey, Slice>> data) {
        int count = 0;
    ...
    private void assertEndAndClose(InternalIterator iter) throws IOException {
        ...
        Assert.assertThrows(
                () ->
                        iter.seek(
                                new InternalKey(
                                        new Slice("k1".getBytes()), 100, ValueType.DELETION)));
    }

    protected final ConcurrentSkipListMap<InternalKey, Slice> getMapOf(
            List<Map.Entry<InternalKey, Slice>> entries) {
        ConcurrentSkipListMap<InternalKey, Slice> table =
                new ConcurrentSkipListMap<>(new InternalKeyComparator(new BytewiseComparator()));
    ...
    private Map.Entry<InternalKey, Slice> entry(String k, int sec, ValueType vt, String v) {
        return new AbstractMap.SimpleEntry<>(
                new InternalKey(new Slice(k.getBytes()), sec, vt), new Slice(v.getBytes()));
    }
}
