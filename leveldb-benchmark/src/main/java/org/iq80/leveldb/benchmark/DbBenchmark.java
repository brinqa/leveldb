...
public class DbBenchmark {
    public static final String FACTORY_CLASS =
            System.getProperty("leveldb.factory", "org.iq80.leveldb.impl.Iq80DBFactory");
    ...
    private void printHeader() throws IOException {
        ...
        System.out.printf(
                "Values:     %d bytes each (%d bytes after compression)%n",
                valueSize, (int) (valueSize * compressionRatio + 0.5));
        System.out.printf("Entries:    %d%n", num);
        System.out.printf(
                "RawSize:    %.1f MB (estimated)%n", ((kKeySize + valueSize) * num) / 1048576.0);
        System.out.printf(
                "FileSize:   %.1f MB (estimated)%n",
    ...
    private void write(ThreadState thread, boolean seq) throws IOException {
    ...
    private void lz4Compress(ThreadState thread) {
        ...
        while (bytes < 1024 * 1048576) { // Compress 1G
            try {
                int compressedSize = Compression.compress(raw, 0, raw.length, compressedOutput, 0);
                bytes += raw.length;
    ...
    public static void main(String[] args) throws Exception {
        ...
        for (String arg : args) {
            ...
            if (arg.startsWith("--")) {
                try {
                    ImmutableList<String> parts =
                            ImmutableList.copyOf(Splitter.on("=").limit(2).split(arg.substring(2)));
    ...
    private class Stats {
        ...
        void report(String name) {
            ...
            System.out.printf(
                    "%-12s : %11.5f micros/op; %11.0f op/sec;%s%s%n",
                    name,
